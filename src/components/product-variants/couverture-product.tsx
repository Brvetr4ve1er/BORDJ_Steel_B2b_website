     ...
     <div className="bg-background p-8 font-sans text-foreground">
       <div className="max-w-7xl mx-auto">
         ...
         <div className="border-l-8 border-accent pl-4 mb-8">
           <h1 className="text-4xl font-bold text-accent uppercase">
             ■ 1-PANNEAUX SANDWICHS DE COUVERTURE
           </h1>
           <p className="text-2xl text-muted-foreground">(à 05 ondes Ép de 30 mm jusqu'au 60 mm)</p>
         </div>
          ...
          <div className="space-y-12">
           ...
           <div className="space-y-12">
             ...
             <h2 className="text-3xl font-bold text-accent mb-4">CARACTÉRISTIQUE PRODUIT</h2>

             {/* Utilisation */}
             <div className="mb-6">
               <h3 className="font-bold text-2xl mb-2">Utilisation</h3>
               <ul className="text-xl space-y-1 list-disc list-inside">
              ...
              {/* Definition */}
             <div className="mb-6">
               <h3 className="font-bold text-2xl mb-2">Definition</h3>
               <p className="text-xl mb-2">
                 Identification d'acier : Nuance S250,<br />
                 S280
               </p>
               <p className="text-xl mb-1 font-semibold">Parement externe</p>
               <p className="text-xl mb-1 list-item ml-4">Profil type: 5 .40 .1000 mm</p>
               <p className="text-xl mb-1">4 ondes profondes et 1 onde sans<br />mousse</p>
               <p className="text-xl mb-2">Épaisseur : 0.5mm - 0.6 mm - 0.7 mm</p>
               <p className="text-xl mb-1 font-semibold">Parement interne</p>
               <p className="text-xl mb-1 list-item ml-4">Profil à nervuration en faible<br />profondeur</p>
               <p className="text-xl">Épaisseur : 0.5mm - 0.6 mm - 0.7 mm</p>
             </div>

              {/* Revêtement */}
             <div className="mb-6">
               <h3 className="font-bold text-2xl mb-2">Revêtement :</h3>
               <p className="text-xl">polyester pour la face extérieure : 25 µm</p>
               <p className="text-xl">polyester pour la face intérieure : 7µm</p>
             </div>

              {/* Âme isolante */}
             <div className="mb-6">
               <h3 className="font-bold text-2xl mb-2">Âme isolante : <span className="font-normal">Mousse polyuréthane</span></h3>
               <p className="text-xl">rigide sans CFC (avec du N-Pentane)</p>
               <p className="text-xl">Conductivité thermique : 0.023 W/m. °c</p>
               <p className="text-xl">Densité (kg/m³) = 38/41 kg m3</p>
             </div>

              {/* Réaction au feu */}
             <div className="mb-6">
               <h3 className="font-bold text-2xl mb-2">Réaction au feu</h3>
               <p className="text-xl">B3 : standard</p>
               <p className="text-xl">B-S2-d0.</p>
             </div>

              {/* Tolérance sur panneaux */}
             <div className="mb-6">
               <h3 className="font-bold text-2xl mb-2">Tolérance sur panneaux</h3>
               <p className="text-xl">Sur épaisseur ±3mm</p>
               <p className="text-xl">Sur longueur ± 3mm</p>
               <p className="text-xl">Sur largeur ± 3mm</p>
               <p className="text-xl">Sur équerrage ± 3mm</p>
             </div>

              {/* Dimensionnement du panneau */}
             <div className="mb-6">
               <h3 className="font-bold text-2xl mb-3">Dimensionnement du panneau</h3>
               <div className="overflow-x-auto">
                 <Table>
                   <TableHeader>
                     <TableRow className="bg-accent text-accent-foreground">
                       {(dimensionsData[0] as string[]).map((header: string, index: number) => <TableHead key={index} className="text-accent-foreground text-xl" dangerouslySetInnerHTML={{ __html: header }}></TableHead>)}
                     </TableRow>
                   </TableHeader>
                   <TableBody>
                    {(dimensionsData.slice(1) as string[][]).map((row: string[], rowIndex: number) => (
                      <TableRow key={rowIndex} className="bg-secondary/30">
                        {row.map((cell: string, cellIndex: number) => <TableCell key={cellIndex} className="text-center text-xl">{cell}</TableCell>)}
                      </TableRow>
                    ))}
                   </TableBody>
                 </Table>
             ...
             <div className="mb-6">
               <h3 className="font-bold text-2xl mb-3">Coefficient d'isolation thermique</h3>
               <Table>
                 <TableHeader>
                   <TableRow className="bg-accent/20">
                    {(thermalData[0] as string[]).map((header: string, index: number) => (
                        <TableHead key={index} className="text-accent text-xl" dangerouslySetInnerHTML={{ __html: header.replace(' ', '<br/>') }}></TableHead>
                    ))}
                   </TableRow>
                 </TableHeader>
                 <TableBody>
                    {(thermalData.slice(1) as string[][]).map((row: string[], rowIndex: number) => (
                        <TableRow key={rowIndex}>
                            {row.map((cell: string, cellIndex: number) => (
                                <TableCell key={cellIndex} className={`text-center text-xl ${cellIndex === 0 ? 'bg-secondary/30 font-semibold' : ''}`}>{cell}</TableCell>
                            ))}
                        </TableRow>
                    ))}
                 </TableBody>
             ...
             <div className="mb-6">
               <h3 className="font-bold text-2xl mb-2">Étanchéité des rives</h3>
              ...
              {/* Sens des panneaux */}
             <div className="mb-6">
               <h3 className="font-bold text-2xl mb-2">Sens des panneaux</h3>
               <p className="text-xl text-justify">
             ...
             <div className="mb-6">
               <h3 className="font-bold text-2xl mb-3">La pose de panneaux sandwichs</h3>

               <h4 className="font-semibold text-xl mb-2">Découpage des panneaux :</h4>
               <p className="text-xl mb-4">
                 Pour avoir un bon résultat il est conseillé d'utiliser
                  une scie sauteuse électrique à lame d'acier.
               </p>

                <h4 className="font-semibold text-xl mb-2">Fixation des panneaux :</h4>
             ...
             <div className="mb-6">
               <h3 className="font-semibold text-2xl mb-3">Caractéristiques Géométriques</h3>
