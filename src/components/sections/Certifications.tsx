 ...
 *
 * Content is read verbatim from `companyData.pages.certifications`, whose
 * `items` are the structured `Certification` records shared with
 * /about/history and the Media Center's ISO tab — so the three pages can no
 * longer disagree about what the client is certified for. The seal shows
 * `code` and `year` inside the ring and `scope` below the plate. The seal
 * geometry — outer ring, dashed inner ring, cardinal tick marks, and the
 * per-certificate pictogram (bullseye for quality, leaf for environment, hard
 * hat for safety, bound through the total `ISO_PICTOGRAMS` map) — is drawn
 * inline so it matches the rest of the site's technical-drawing vernacular and
...
/* --------------------------------------------------------------------------
 * Seal geometry — shared by the pictograms and the seal itself so a figure can
 * never drift off the ring's centre.
 * -------------------------------------------------------------------------- */
...
/* --------------------------------------------------------------------------
 * Per-certificate pictogram — drawn behind the ISO type, kept low-opacity so
 * the text always wins. Small, hand-drawn feel; no bitmaps.
 * -------------------------------------------------------------------------- */

/** Bullseye — the QMS "target" motif. */
function QualityPictogram() {
  const cx = SEAL_CX;
  const cy = SEAL_CY;
...
type SealPictogram = () => React.JSX.Element;

/**
 * Total over `CertificationCode`, so this map is what keeps config and artwork
 * in step: add a certification in company-data without drawing its pictogram
 * here and the build fails, rather than shipping a seal with an empty middle.
 */
const ISO_PICTOGRAMS: Record<CertificationCode, SealPictogram> = {
  'iso-9001': QualityPictogram,
  'iso-14001': EnvironmentPictogram,
  'iso-45001': SafetyPictogram,
};
...
function IsoSeal({
  ...
  const Pictogram = ISO_PICTOGRAMS[cert.id];
  ...
  return (
    <svg
      ...
      role="img"
      aria-label={`Sceau ${cert.code}:${cert.year}`}
    ...
    >
      ...
      {/* Certificate pictogram sits behind the geometry. */}
      <Pictogram />
